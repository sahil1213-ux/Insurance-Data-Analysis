# Insurance-Analysis

### Dashboard Link: https://app.powerbi.com/groups/e271dca1-67f2-46e7-be89-d6337a1fccad/dashboards/a24ae137-54dc-4b24-abc1-092189455900?ctid=00436dcb-a0dc-4a68-a066-91c516efd0d4&pbi_source=linkShare

### Report Link:  https://app.powerbi.com/links/Jo4z5JOtmW?ctid=00436dcb-a0dc-4a68-a066-91c516efd0d4&pbi_source=linkShare


## Problem Statement

This dashboard helps insurance companies better understand their customers and policy trends. It enables insurers to analyze claim patterns, assess risk factors, and optimize premium pricing. By identifying the key factors influencing claim approvals and rejections, insurers can improve their claim processing efficiency and detect potential fraud.  

Additionally, the dashboard provides insights into customer demographics, policy preferences, and claim frequencies, helping insurers develop data-driven strategies for customer retention. Since many claims are either pending or rejected, insurers must streamline their approval processes and enhance transparency.  

Furthermore, analyzing the relationship between coverage amounts and claim frequencies allows insurers to refine their risk assessment models. By leveraging these insights, insurance companies can enhance their services, reduce fraudulent claims, and improve customer satisfaction.
 
# Snapshot of Dashboard (Power BI Service)

![Image](https://github.com/user-attachments/assets/9f1a7f61-5027-425e-9b44-b299a8a5ab87)

 
 # Report Snapshot (Power BI DESKTOP)

![Image](https://github.com/user-attachments/assets/198afaa2-44dc-4bcf-af46-8d95166403d8)

# Sentiment Analysis Report (Power BI DESKTOP)
![Image](https://github.com/user-attachments/assets/d0827fca-6d87-4c1e-9c7f-df2a6c70b406)
# Insights

# Insurance Data Insights Report

## **Insights from the Dashboard**  

---

## **1. Total Number of Policies = 10,000**  
- **Total number of claims approved** = 3386 (33.86%)
- **Total number of claims rejected** = 4354 (43.54%)
- **Total number of pending claims** = 2260 (22.60%)

➡ A significant portion of claims are either **pending or rejected**, indicating a need to streamline claim approvals.  


---

## **2. Customer Demographics**  

- **Male policyholders** = 5,000
- **Female policyholders** = 4,965  
- **Age Group Distribution:**  
- **61+**: 38.19%
- **31-45**: 21.70%
- **46-60**: 21.22%
- **19-30**: 17.44%
- **0-18**: 1.45%


➡ The **60+ years age group** has the highest number of policyholders.  


---

## **3. Policy Distribution by Type**  

- **Travel** = 4145 (41.45%)
- **Health** = 2000 (20.00%)
- **Auto** = 1594 (15.94%)
- **Life** = 1248 (12.48%)
- **Home** = 1013 (10.13%)

➡ **Travel** policies are the most common, followed by health insurance.  


---

## **4. Average Premium & Coverage Amount**  

- **Average Premium Amount** = 597.41
- **Average Coverage Amount** = 60033.12

➡ Premium pricing varies depending on policy type and risk factors.  

---

## **5. Claims & Settlement Analysis**  

- **Average Claim Amount (Approved Claims)** = ₹2984.37
- **Average Claim Amount (Rejected Claims)** = ₹0.00
- **Pending claims have an average amount of ₹3008.51**

➡ Higher claim amounts tend to have a **higher pending rate**.  


---

## **6. Claim Status by Policy Type**  

- **Auto:** 22.40% Pending, 42.22% Rejected, 35.38% Settled
- **Health:** 22.90% Pending, 43.35% Rejected, 33.75% Settled
- **Home:** 20.93% Pending, 46.20% Rejected, 32.87% Settled
- **Life:** 23.72% Pending, 45.35% Rejected, 30.93% Settled
- **Travel:** 22.61% Pending, 42.94% Rejected, 34.45% Settled

➡ **Auto Insurance** has the highest claim approval rate, while **Home Insurance** has the most rejections.  


---
---

By analyzing these insights, Prism Insurance PVT LTD can enhance claim processing efficiency, reduce fraud risks, and optimize customer engagement strategies.

